"""
Apply Engine용 안전한 파일 작업 계층.

파일 정리 실행 시 승인된 작업만 루트 이내에서 수행. 경로 검증, 심볼릭 링크·깊이 제한.
이해→판단→계획→실행→Undo 흐름 중 '실행' 단계의 안전 보장.
"""
...
class SafeFileOperations:
    """
    Apply Engine 파일 작업 래퍼.

    사용자 지정 루트 이내 경로만 허용(외부 쓰기 방지). 심볼릭 링크·깊이 제한.
    rename/move/mkdir 등 정리 작업을 안전하게 수행.
    """
